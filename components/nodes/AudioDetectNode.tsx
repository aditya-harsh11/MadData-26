...
export default function AudioDetectNode({ id, selected, data }: NodeProps) {
  ...
  // Ref to hold latest filter labels for use in handler
  ...
  // Listen for audio detection results from backend — write outputs and restart recording
  useEffect(() => {
    const handler = (payload: any) => {
      ...
      // Compute filtered detections inline (not waiting for React re-render)
      ...
      if (!retriggerRef.current && outputText === lastOutputRef.current) {
        // Skip duplicate output when retrigger is off
      } else {
  ...
  // Main loop: poll audio store, accumulate 1s chunks, send when we have enough
  useEffect(() => {
    if (!connectedMicId) {
      setPhase("idle");
      return;
    ...
    const pollInterval = setInterval(() => {
      ...
      if (chunksRef.current.length >= listenDuration) {
        // Concatenate all recorded chunks into one buffer
  ...
  // Safety timeout: if stuck in analyzing for >10s, force restart
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Analyzing indicator */}
      {phase === "analyzing" && (
        <div className="flex items-center gap-2 text-sm text-violet-400 mb-3">
      ...
      <label className="flex items-center gap-2 mb-3 cursor-pointer nodrag select-none">
        <button
          ...
          style={{
            background: retrigger ? "#8b5cf640" : "#1e1e2e",
            border: `1px solid ${retrigger ? "#8b5cf650" : "#2a2a3a"}`,
          }}
        >
          <div
            className="absolute top-[2px] w-3 h-3 rounded-full transition-all"
            style={{
              background: retrigger ? "#8b5cf6" : "#64748b",
              left: retrigger ? 14 : 2,
            }}
          />
        </button>
